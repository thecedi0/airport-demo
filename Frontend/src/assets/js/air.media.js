$(document).ready(function() {
            ...
            for (let i = 0; i < slidesLength; i++) {
              if (i == currentSlide) {
                ...
                setTimeout(function() {
                  ...
                  $('#' + slideId)
                    .find('.ad-slide')
                    [i].classList.remove('exit');
                }, 1050);

                $('#' + slideId)
                  .find('.ad-slide-bullet span')
                  [i].classList.remove('active');
  ...
  function scanDOM4media() {
    ...
    if (wrapper.find('.ad-video-remix').not('[_adConstructed]').length !== 0) {
      console.log(
        'video foound',
        $('.ad-video-remix').not('[_adConstructed]').length
      );
      constructVideoRemix();
    }
  ...
  function carouselNav(clicked, factor) {
    ...
    var moveFactor = -1;
    if (parent[0].hasAttribute('paginate')) {
      moveFactor = -1 * visibleItems;
    }
    const moveBy = parseFloat(parent.attr('moveby')) * -1;

    const caltems = lHiddenItems + visibleItems;
    let calMove = 0;
    let updateHidden = 0;

    if (
      (factor === -1 && caltems >= totalItems) ||
      (factor === 1 && caltems <= visibleItems)
    ) {
      // console.log('maximum reached, so dont move again, or hide it. Hidden, Visible, Total', lHiddenItems, visibleItems, totalItems);
    } else {
      if (factor === -1) {
        calMove = (lHiddenItems + Math.abs(moveFactor)) * moveBy;
        updateHidden = lHiddenItems + Math.abs(moveFactor);
      } else {
      ...
      // console.log('calMove is -->', calMove);
      parent.find('.carousel-overflow').animate(
        {
          'margin-left': calMove.toString() + 'px'
        },
        300,
        'linear'
      );
  ...
  function constructAudio() {
    $('.ad-audio')
      ...
      .each(function() {
        ...
        audioPlayerId = $(this)[0].hasAttribute('audioId')
          ? '#' + $(this).attr('audioId')
          : '#audio';
  ...
  wrapper.on('click', '.ad-audio button.play', function(e) {
    ...
    let audioPlayerId = parent[0].hasAttribute('audioId')
      ? '#' + parent.attr('audioId')
      : '#audio';
    let icon = $(this).find('i.fa');
    // console.log(parent.attr('isplaying'));
